...
let gravatar = require('gravatar');//生成头像
let path = require('path');
let favicon = require('serve-favicon');
let compression = require('compression')// gzip压缩
...
let xss = require('xss')
let marked = require('marked');//解析markdown
let strip = require('strip');//提取html中文本


marked.setOptions({
    renderer: new marked.Renderer(),
    gfm: true,//If true, use approved GitHub Flavored Markdown (GFM) specification.
    tables: true,//If true and gfm is true, use GFM Tables extension.   
    breaks: false,//If true, add <br> on a single line break (copies GitHub). Requires gfm be true.
...
//定义全局字段
app.locals = {
    title: config.title || 'express+mongodb 博客',
    keywords:'node.js,express,js全栈,博客系统,mongodb数据库',
    description:'采用express+mongodb的多用户博客系统',
    pretty: true,
    moment: moment,
    _: _,
    core: core,
...
//路由控制
core.walk(appPath + '/routes/app', null, function(path) {
    require(path)(app);
});
core.walk(appPath + '/routes/server', null, function(path) {
    require(path)(app);
});
